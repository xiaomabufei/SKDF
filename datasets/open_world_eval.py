...
class OWEvaluator:
    ...
    def accumulate(self):
        for class_label_ind, class_label in enumerate(self.voc_gt.CLASS_NAMES):
            ...
            self.num_all_preditions += len(lines_by_class)
            ovthresh = 50
            ...
            self.rec, self.prec, self.AP[class_label_ind, ovthresh_ind], self.unk_det_as_known, \
                self.num_unk, self.tp_plus_fp_closed_set, self.fp_open_set, self.num_known_det_as_unk, self.num_unknown_rest, self.tp_fp = voc_eval(lines_by_class, \
            ...
            if class_label == 'unknown':
                self.num_known_det_as_unks[ovthresh] = self.num_known_det_as_unk
    ...
    def summarize(self, fmt='{:.06f}'):
        ...
        total_num_known_det_as_unk = self.num_known_det_as_unks[50]
        error_detection_rate = (total_num_unk_det_as_known[50] + total_num_known_det_as_unk) / self.num_all_preditions
        num_unknown_rest = self.num_unknown_rest

        print('Absolute CSE (total_num_known_det_as_unk): ' + str(total_num_known_det_as_unk))
        print('Absolute OSE (total_num_unk_det_as_known): ' + str(total_num_unk_det_as_known))
        print('error_detection_rate: ' + str(error_detection_rate))        
        print('num_all_preditions: ' + str(self.num_all_preditions))
        print('total_num_unk ' + str(total_num_unk))
        print('num_unknown_rest ' + str(num_unknown_rest))
        ...
        self.metrics['CSE'] = str(total_num_known_det_as_unk)
        self.metrics['error_detection_rate'] = str(error_detection_rate)
        self.metrics['num_unknown_rest'] = str(num_unknown_rest)
        ...
        self.metrics["Current_AP"] = str(self.AP[:, o50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls].mean()) # tensorboard
        ...
        self.metrics["Both"] = str(self.AP[:, o50][:self.prev_intro_cls + self.curr_intro_cls].mean()) # tensorboard
        ...
        self.metrics["Unknown_Precisions50"] =  str(self.precs[50][-1])
        self.metrics["Unknown Recall50: "] = str(self.recs[50][-1])
        self.metrics["U_Recall"] = str(self.recs[50][-1]) # tensorboard

        if len(self.tp_fp[0]) == 0:
            self.metrics["TP"] = 0
            self.metrics["FP"] = 0
            self.metrics["num_below03"] = 0
        else:
            self.metrics["TP"] = str(self.tp_fp[0][-1])
            self.metrics["FP"] = str(self.tp_fp[1][-1])
            self.metrics["num_below03"] = str(self.tp_fp[2])
        ...
        for class_name, ap in zip(self.voc_gt.CLASS_NAMES, self.AP[:, o50].cpu().tolist()):
            ...
            if class_name == 'unknown':
                self.metrics['unknown_ap'] = str(fmt.format(ap))
...
def voc_eval(detpath,
    ...
    if classname == 'unknown':
        tp_fp = [tp, fp, num_unknown_below03]
...
#-----------------------------------------------------------------------------------
    # known detected as unknown
    if classname == 'unknown':
        n_known_gt = 0
        all_class_recs = {}
        all_npos = 0
        for imagename in imagenames:
            R = [obj for obj in recs[imagename] if obj['name'] != 'unknown' ]  # all known gt
            bbox = np.array([x['bbox'] for x in R])
            difficult = np.array([x['difficult'] for x in R]).astype(np.bool)
            det = [False] * len(R)
            n_known_gt += len(R)
            all_npos = all_npos + sum(~difficult)
            all_class_recs[imagename] = {'bbox': bbox,
                                    'difficult': difficult,
                                    'det': det}

        is_known = np.zeros(nd)
        for d in range(nd):
            R = all_class_recs[image_ids[d]]  # known GT 
            bb = BB[d, :].astype(float)  # unknown det  
            ovmax = -np.inf
            BBGT = R["bbox"].astype(float)  # known GT 
            ...
            if ovmax > ovthresh:
                is_known[d] = 1.0
        is_known_sum = np.sum(is_known)


#-----------------------------------------------------------------------------------
        return rec, prec, ap, 0., n_unk, None, None, is_known_sum, num_unknown_rest, tp_fp
    ...
    for d in range(nd):
        ...
        bb = BB[d, :].astype(float) # class det  
        ovmax = -np.inf
